...
class MainWindow(QtWidgets.QMainWindow):
    ...
    tabs (Dashboard, Settings) and controls the threads responsible for listening to packets
    """
    ...
    def __init__(self):
        ...
        # Connect the signals to change tabs
        self.dashboard.settingsButton.clicked.connect(self.changeToSettingsTab)
        self.settings.saveButton.clicked.connect(self.changeToDashboardTab)
        self.settings.cancelButton.clicked.connect(self.changeToDashboardTab)

        # So the listen button triggers the thread
        self.dashboard.listenButton.clicked.connect(self.toggle_loop)

        # Chain the update signal to the dashboard widget's update signal
        self.updateSignal.connect(self.dashboard.updateSignal)

        # Show or hide the 'NOT RACING' alert based on fdp.is_race_on
        self.isRacing.connect(self.dashboard.notRacing.showHide)
    ...
    @Slot()
    def changeToDashboardTab(self):
        """Changes the current tab to the dashboard tab"""
        self.stack.setCurrentIndex(self.TabIndex.DASHBOARD.value)
    ...
    def toggle_loop(self, checked):
        ...
        if not checked:
            ...
            self.dashboard.listenButton.setEnabled(False)

            # Change the buttons back to normal
            self.dashboard.listenButton.setStyleSheet("color: white;")
            self.dashboard.settingsButton.setStyleSheet("color: white;")
            self.dashboard.resetButton.setStyleSheet("color: white;")
            self.dashboard.listenButton.setText("START")
            ...
            self.dashboard.settingsButton.setEnabled(False)
            self.dashboard.resetButton.setEnabled(False)

            # Change buttons to a darker colour while listening to avoid
            # distracting the player
            self.dashboard.listenButton.setStyleSheet("color: #3f3f3f;")
            self.dashboard.settingsButton.setStyleSheet("color: #3f3f3f;")
            self.dashboard.resetButton.setStyleSheet("color: #3f3f3f;")

            # Change listen button to 'stop'
            self.dashboard.listenButton.setText("STOP")
    ...
    def loop_finished(self):
        ...
        self.dashboard.listenButton.setEnabled(True)
        self.dashboard.settingsButton.setEnabled(True)
        self.dashboard.resetButton.setEnabled(True)
